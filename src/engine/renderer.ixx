...
public:
  Renderer(platform::WindowHandle window) {
    ...
    m_surface = platform::create_vulkan_surface(m_instance, window);
    ...
    for (auto& sem : m_semaphores) {
      ...
      vkCreateSemaphore(m_device, &semaphore_info, nullptr, &sem);
    }
    ...
    for (auto& cmd_buf : m_command_buffers) {
      ...
      if (vkAllocateCommandBuffers(m_device, &cmd_buf_info, &cmd_buf) != VK_SUCCESS) {
